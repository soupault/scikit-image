...
def test_view_as_blocks_block_not_a_tuple():
    A = np.arange(10)
    assert_raises(TypeError, view_as_blocks, A, [5])


def test_view_as_blocks_negative_shape():
    A = np.arange(10)
    assert_raises(ValueError, view_as_blocks, A, (-2,))


def test_view_as_blocks_block_too_large():
    A = np.arange(10)
    assert_raises(ValueError, view_as_blocks, A, (11,))
...
def test_view_as_blocks_1D_array_wrong_block_shape():
    A = np.arange(10)
    assert_raises(ValueError, view_as_blocks, A, (3,))
...
def test_view_as_windows_input_not_array():
    A = [1, 2, 3, 4, 5]
    assert_raises(TypeError, view_as_windows, A, (2,))
...
def test_view_as_windows_negative_window_length():
    A = np.arange(10)
    view_as_windows(A, (-1,))
    assert_raises(ValueError, view_as_windows, A, (-1,))


def test_view_as_windows_window_too_large():
    A = np.arange(10)
    view_as_windows(A, (11,))
    assert_raises(ValueError, view_as_windows, A, (11,))


def test_view_as_windows_step_below_one():
    A = np.arange(10)
    view_as_windows(A, (11,), step=0.9)
    assert_raises(ValueError, view_as_windows, A, (11,), step=0.9)
